...
// DELETED BY DAWSON -- We'll expose these as properties inside of JMCRequestQueue so we can control the transports ourselves, programatically.
...
// ADDED BY DAWSON -- Expose the transports so we can control them. In particular, we want to control their delegates. The existing library doesn't allow us to do this.
JMCIssueTransport* _issueTransport;
JMCReplyTransport* _replyTransport;

@synthesize issueTransport = _issueTransport;
@synthesize replyTransport = _replyTransport;
...
+ (JMCRequestQueue *)sharedInstance
{
    ...
    if (instance == nil) {
		...
		// DELETED BY DAWSON -- Don't initialize the transports; we'll do that elsewhere. We want to be able to specify our own delegate for them.
...
-(BOOL) reachable {
    JMCReachability *r = [JMCReachability reachabilityWithHostName:[JMC instance].url.host];
...
-(void) flushQueue
{
	// CHANGED BY DAWSON -- Directly flush the queue rather than throwing it on a timer for later.
	// (Some interaction of our threading in Titanium was preventing this from ever getting through if left as existing…)
    [self flushQueueIfReachable:nil];
}
