...
+(void)addAllAttachments:(NSArray *)allAttachments toRequest:(JMCASIFormDataRequest *)upRequest
{
...
{
    // only JMCASIFormDataRequest are queued at the moment...
    NSURL *url = [self makeUrlFor:item.originalIssueKey];
    if (!url) {
        JMCALog(@"Invalid URL made for original issue key: %@", item.originalIssueKey);
        return nil;
    }
    JMCASIFormDataRequest* request = [JMCASIFormDataRequest requestWithURL:url];
