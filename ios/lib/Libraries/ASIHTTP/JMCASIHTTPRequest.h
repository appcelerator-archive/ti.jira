...
@class JMCASIDataDecompressor;

extern NSString *JMCASIHTTPRequestVersion;
...
typedef enum _ASIAuthenticationState {
	JMCASINoAuthenticationNeededYet = 0,
	JMCASIHTTPAuthenticationNeeded = 1,
	JMCASIProxyAuthenticationNeeded = 2
} JMCASIAuthenticationState;

typedef enum _ASINetworkErrorType {
    JMCASIConnectionFailureErrorType = 1,
    JMCASIRequestTimedOutErrorType = 2,
    JMCASIAuthenticationErrorType = 3,
    JMCASIRequestCancelledErrorType = 4,
    JMCASIUnableToCreateRequestErrorType = 5,
    JMCASIInternalErrorWhileBuildingRequestType  = 6,
    JMCASIInternalErrorWhileApplyingCredentialsType  = 7,
	JMCASIFileManagementError = 8,
	JMCASITooMuchRedirectionErrorType = 9,
	JMCASIUnhandledExceptionError = 10,
	JMCASICompressionError = 11
	
} JMCASINetworkErrorType;


// The error domain that all errors generated by JMCASIHTTPRequest use
extern NSString* const JMCNetworkRequestErrorDomain;
...
extern unsigned long const JMCASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^JMCASIBasicBlock)(void);
typedef void (^JMCASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^JMCASISizeBlock)(long long size);
typedef void (^JMCASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^JMCASIDataBlock)(NSData *data);
#endif

@interface JMCASIHTTPRequest : NSOperation <NSCopying> {
	...
	// The delegate - will be notified of various changes in state via the JMCASIHTTPRequestDelegate protocol
	id <JMCASIHTTPRequestDelegate> delegate;
	
	// Another delegate that is also notified of request status changes and progress updates
	// Generally, you won't use this directly, but JMCASINetworkQueue sets itself as the queue so it can proxy updates to its own delegates
	// NOTE: WILL BE RETAINED BY THE REQUEST
	id <JMCASIHTTPRequestDelegate, JMCASIProgressDelegate> queue;
	...
	// Automatically set to true in JMCASIFormDataRequests when using setFile:forKey:
	...
	// Set to true when JMCASIHTTPRequest automatically created a temporary file containing the request body (when true, the file at postBodyFilePath will be deleted at the end of the request)
	...
	// If error code is = JMCASIConnectionFailureErrorType (1, Connection failure occurred) - inspect [[error userInfo] objectForKey:NSUnderlyingErrorKey] for more information
	NSError *error;
	...
	id <JMCASIProgressDelegate> uploadProgressDelegate;
	...
	id <JMCASIProgressDelegate> downloadProgressDelegate;
	...
	// If you are using Basic authentication and want to force JMCASIHTTPRequest to send an authorization header without waiting for a 401, you must set this to (NSString *)kCFHTTPAuthenticationSchemeBasic
	NSString *authenticationScheme;
	
	// Realm for authentication when credentials are required
	NSString *authenticationRealm;
	
	// When YES, JMCASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a server that requires authentication
	...
	// When YES, JMCASIHTTPRequest will present a dialog allowing users to enter credentials when no-matching credentials were found for a proxy server that requires authentication
	...
	// If you set this and implement the method in your delegate, you must handle the data yourself - JMCASIHTTPRequest will not populate responseData or write the data to downloadDestinationPath
	...
	JMCASIHTTPRequest *mainRequest;
	
	// When NO, this request will only update the progress indicator when it completes
	// When YES, this request will update the progress indicator according to how much data it has received so far
	// The default for requests is YES
	// Also see the comments in JMCASINetworkQueue.h
	...
	// Tells JMCASIHTTPRequest not to delete partial downloads, and allows it to use an existing file to resume a download. Defaults to NO.
	...
	// Details on the proxy to use - you could set these yourself, but it's probably best to let JMCASIHTTPRequest detect the system proxy settings
	NSString *proxyHost;
	int proxyPort;
	
	// JMCASIHTTPRequest will assume kCFProxyTypeHTTP if the proxy type could not be automatically determined
	...
	// See JMCASIAuthenticationState values above. 0 == default == No authentication needed yet
	JMCASIAuthenticationState authenticationNeeded;
	
	// When YES, JMCASIHTTPRequests will present credentials from the session store for requests to the same server before being asked for them
	...
	// Temporarily set to YES when a closed connection forces a retry (internally, this stops JMCASIHTTPRequest cleaning up a temporary post body)
	...
	// Will be JMCASIHTTPRequestRunLoopMode for synchronous requests, NSDefaultRunLoopMode for all other requests
	NSString *runLoopMode;
	
	// This timer checks up on the request every 0.25 seconds, and updates progress
	NSTimer *statusTimer;
	
	// The download cache that will be used for this request (use [JMCASIHTTPRequest setDefaultCache:cache] to configure a default cache
	id <JMCASICacheDelegate> downloadCache;
	
	// The cache policy that will be used for this request - See JMCASICacheDelegate.h for possible values
	JMCASICachePolicy cachePolicy;
	
	// The cache storage policy that will be used for this request - See JMCASICacheDelegate.h for possible values
	JMCASICacheStoragePolicy cacheStoragePolicy;
	...
	JMCASIDataDecompressor *dataDecompressor;
	...
	//
	// Setting this to NO may be especially useful for users using JMCASIHTTPRequest in conjunction with a streaming parser, as it will allow partial gzipped responses to be inflated and passed on to the parser while the request is still running
	...
	JMCASIHTTPRequest *PACFileRequest;
	...
	JMCASIBasicBlock startedBlock;

	//block to execute when headers are received
	JMCASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	JMCASIBasicBlock completionBlock;

	//block to execute when request fails
	JMCASIBasicBlock failureBlock;

	//block for when bytes are received
	JMCASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	JMCASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	JMCASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	JMCASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	JMCASIDataBlock dataReceivedBlock;

	//block for handling authentication
	JMCASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	JMCASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    JMCASIBasicBlock requestRedirectedBlock;
	#endif
}
...
// Used internally to record bandwidth use, and by JMCASIInputStreams when uploading. It's probably best if you don't mess with this.
+ (void)incrementBandwidthUsedInLastSecond:(unsigned long)bytes;

// On iPhone, JMCASIHTTPRequest can automatically turn throttling on and off as the connection type changes between WWAN and WiFi
...
@property (assign) JMCASIHTTPRequest *mainRequest;
...
@property (assign, readonly) JMCASIAuthenticationState authenticationNeeded;
...
@property (assign) id <JMCASICacheDelegate> downloadCache;
@property (assign) JMCASICachePolicy cachePolicy;
@property (assign) JMCASICacheStoragePolicy cacheStoragePolicy;
...
#endif
@property (retain) JMCASIDataDecompressor *dataDecompressor;
