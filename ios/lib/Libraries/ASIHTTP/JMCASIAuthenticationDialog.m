...
static JMCASIAuthenticationDialog *sharedDialog = nil;
...
@implementation JMCASIAutorotatingViewController
...
@interface JMCASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation JMCASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [JMCASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(JMCASIHTTPRequest *)theRequest
{
	...
	if (!sharedDialog) {
		...
		if ([theRequest authenticationNeeded] == JMCASIProxyAuthenticationNeeded) {
			[sharedDialog setType:JMCASIProxyAuthenticationType];
		} else {
			[sharedDialog setType:JMCASIStandardAuthenticationType];
		}
		[sharedDialog show];
	} else {
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[JMCASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)showTitle
{
	...
	if (UIInterfaceOrientationIsPortrait([[UIDevice currentDevice] orientation])) {
		// Setup the title
		if ([self type] == JMCASIProxyAuthenticationType) {
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (JMCASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (JMCASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		JMCASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
...
- (void)loginWithCredentialsFromDialog:(id)sender
{
	for (JMCASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
		...
		// Handle NTLM domains
		NSString *scheme = ([self type] == JMCASIStandardAuthenticationType) ? [[self request] authenticationScheme] : [[self request] proxyAuthenticationScheme];
