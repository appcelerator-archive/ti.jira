...
#import "JMCASIDownloadCache.h"
#import "JMCASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static JMCASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";

@interface JMCASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation JMCASIDownloadCache

- (id)init
{
	self = [super init];
	[self setShouldRespectCacheControlHeaders:YES];
	[self setDefaultCachePolicy:JMCASIUseDefaultCachePolicy];
...
+ (id)sharedCache
{
	if (!sharedCache) {
		...
		[sharedCache setStoragePath:[[NSSearchPathForDirectoriesInDomains(NSCachesDirectory, NSUserDomainMask, YES) objectAtIndex:0] stringByAppendingPathComponent:@"JMCASIHTTPRequestCache"]];
...
{
	[[self accessLock] lock];
	[self clearCachedResponsesForStoragePolicy:JMCASICacheForSessionDurationCacheStoragePolicy];
	[storagePath release];
...
- (NSDate *)expiryDateForRequest:(JMCASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if (maxAge) {
		...
		if (expires) {
			return [JMCASIHTTPRequest dateFromRFC1123String:expires];
		}
	}
	return nil;
}
...
- (void)storeResponseForRequest:(JMCASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	...
	if ([request error] || ![request responseHeaders] || ([request cachePolicy] & JMCASIDoNotWriteToCacheCachePolicy)) {
		[[self accessLock] unlock];
		return;
	}
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(JMCASIHTTPRequest *)request
{
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:([request cacheStoragePolicy] == JMCASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
{
	[[self accessLock] lock];
	JMCASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(JMCASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
		defaultCachePolicy = JMCASIAskServerIfModifiedWhenStaleCachePolicy;
	}  else {
...
- (void)clearCachedResponsesForStoragePolicy:(JMCASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
	...
	NSString *path = [[self storagePath] stringByAppendingPathComponent:(storagePolicy == JMCASICacheForSessionDurationCacheStoragePolicy ? sessionCacheFolder : permanentCacheFolder)];
...
- (BOOL)canUseCachedDataForRequest:(JMCASIHTTPRequest *)request
{
	...
	if ([request cachePolicy] & JMCASIDoNotReadFromCacheCachePolicy) {
		return NO;

	// If we don't want to load the request whatever happens, always pretend we have cached data even if we don't
	} else if ([request cachePolicy] & JMCASIDontLoadCachePolicy) {
		return YES;
	}
	...
	if ([request cachePolicy] & JMCASIOnlyLoadIfNotCachedCachePolicy) {
		return YES;

	// If we want to fallback to the cache after an error
	} else if ([request complete] && [request cachePolicy] & JMCASIFallbackToCacheIfLoadFailsCachePolicy) {
		return YES;

	// If we have cached data that is current, we can use it
	} else if ([request cachePolicy] & JMCASIAskServerIfModifiedWhenStaleCachePolicy) {
	...
	} else if ([request cachePolicy] & JMCASIAskServerIfModifiedCachePolicy) {
