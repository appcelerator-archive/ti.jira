...
#import "JMCASIAuthenticationDialog.h"
#import <MobileCoreServices/MobileCoreServices.h>
#else
#import <SystemConfiguration/SystemConfiguration.h>
#endif
#import "JMCASIInputStream.h"
#import "JMCASIDataDecompressor.h"
#import "JMCASIDataCompressor.h"

// Automatically set on build
NSString *JMCASIHTTPRequestVersion = @"v1.8-95 2011-05-28";

static NSString *defaultUserAgent = nil;

NSString* const JMCNetworkRequestErrorDomain = @"JMCASIHTTPRequestErrorDomain";

static NSString *JMCASIHTTPRequestRunLoopMode = @"JMCASIHTTPRequestRunLoopMode";
...
const int JMCRedirectionLimit = 5;
...
static void ReadStreamClientCallBack(CFReadStreamRef readStream, CFStreamEventType type, void *clientCallBackInfo) {
    [((JMCASIHTTPRequest*)clientCallBackInfo) handleNetworkEvent: type];
}
...
static NSError *JMCASIRequestCancelledError;
static NSError *JMCASIRequestTimedOutError;
static NSError *JMCASIAuthenticationError;
static NSError *JMCASIUnableToCreateRequestError;
static NSError *JMCASITooMuchRedirectionError;
...
unsigned long const JMCASIWWANBandwidthThrottleAmount = 14800;
...
// When using JMCASIAuthenticationDialogs, it also ensures only one dialog is shown at once
...
static id <JMCASICacheDelegate> defaultCache = nil;

// Used for tracking when requests are using the network
static unsigned int runningRequestCount = 0;

// You can use [JMCASIHTTPRequest setShouldUpdateNetworkActivityIndicator:NO] if you want to manage it yourself
...
- (void)fetchPACFile;
- (void)finishedDownloadingPACFile:(JMCASIHTTPRequest *)theRequest;
...
- (void)performBlockOnMainThread:(JMCASIBasicBlock)block;
- (void)releaseBlocksOnMainThread;
+ (void)releaseBlocks:(NSArray *)blocks;
- (void)callBlock:(JMCASIBasicBlock)block;
#endif
...
@property (assign) JMCASIAuthenticationState authenticationNeeded;
...
@property (retain, nonatomic) JMCASIHTTPRequest *PACFileRequest;
...
@implementation JMCASIHTTPRequest

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [JMCASIHTTPRequest class]) {
		...
		JMCASIRequestTimedOutError = [[NSError alloc] initWithDomain:JMCNetworkRequestErrorDomain code:JMCASIRequestTimedOutErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request timed out",NSLocalizedDescriptionKey,nil]];  
		JMCASIAuthenticationError = [[NSError alloc] initWithDomain:JMCNetworkRequestErrorDomain code:JMCASIAuthenticationErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Authentication needed",NSLocalizedDescriptionKey,nil]];
		JMCASIRequestCancelledError = [[NSError alloc] initWithDomain:JMCNetworkRequestErrorDomain code:JMCASIRequestCancelledErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request was cancelled",NSLocalizedDescriptionKey,nil]];
		JMCASIUnableToCreateRequestError = [[NSError alloc] initWithDomain:JMCNetworkRequestErrorDomain code:JMCASIUnableToCreateRequestErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create request (bad url?)",NSLocalizedDescriptionKey,nil]];
		JMCASITooMuchRedirectionError = [[NSError alloc] initWithDomain:JMCNetworkRequestErrorDomain code:JMCASITooMuchRedirectionErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"The request failed because it redirected too many times",NSLocalizedDescriptionKey,nil]];
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setTimeOutSeconds:[JMCASIHTTPRequest defaultTimeOutSeconds]];
...
+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <JMCASICacheDelegate>)cache
{
	return [self requestWithURL:newURL usingCache:cache andCachePolicy:JMCASIUseDefaultCachePolicy];
}

+ (id)requestWithURL:(NSURL *)newURL usingCache:(id <JMCASICacheDelegate>)cache andCachePolicy:(JMCASICachePolicy)policy
{
	JMCASIHTTPRequest *request = [[[self alloc] initWithURL:newURL] autorelease];
...
- (void)dealloc
{
	[self setAuthenticationNeeded:JMCASINoAuthenticationNeededYet];
...
- (void)buildPostBody
{
	...
	if ([self postBodyFilePath]) {
		...
		if ([self shouldCompressRequestBody]) {
			if (![self compressedPostBodyFilePath]) {
				...
				NSError *err = nil;
				if (![JMCASIDataCompressor compressDataFromFile:[self postBodyFilePath] toFile:[self compressedPostBodyFilePath] error:&err]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",path],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	...
	// Otherwise, we have an in-memory request body
		...
		if ([self shouldCompressRequestBody]) {
			NSError *err = nil;
			NSData *compressedBody = [JMCASIDataCompressor compressData:[self postBody] error:&err];
			if (err) {
				[self failWithError:err];
				return;
			}
...
- (void)cancelOnRequestThread
{
	...
	[self failWithError:JMCASIRequestCancelledError];
...
{	
	if ([self isResponseCompressed] && [self shouldWaitToInflateCompressedResponses]) {
		return [JMCASIDataDecompressor uncompressData:[self rawResponseData] error:NULL];
	} else {
...
- (void)startSynchronous
{
	...
	[self setRunLoopMode:JMCASIHTTPRequestRunLoopMode];
...
- (void)main
{
	@try {
		...
		if ([JMCASIHTTPRequest isMultitaskingSupported] && [self shouldContinueWhenAppEntersBackground]) {
		...
		// A HEAD request generated by an JMCASINetworkQueue may have set the error already. If so, we should not proceed.
		if ([self error]) {
			[self setComplete:YES];
			[self markAsFinished];
			return;		
		...
		if (![self url]) {
			[self failWithError:JMCASIUnableToCreateRequestError];
			return;		
		}
		...
		//If this is a HEAD request generated by an JMCASINetworkQueue, we need to let the main request generate its headers first so we can use them
		if ([self mainRequest]) {
			[[self mainRequest] buildRequestHeaders];
		}
		
		// Even if this is a HEAD request with a mainRequest, we still need to call to give subclasses a chance to add their own to HEAD requests (JMCASIS3Request does this)
		...
		if ([self downloadCache]) {
			...
			if ([self cachePolicy] & (JMCASIAskServerIfModifiedWhenStaleCachePolicy|JMCASIAskServerIfModifiedCachePolicy)) {
		...
		NSError *underlyingError = [NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIUnhandledExceptionError userInfo:[exception userInfo]];
		[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIUnhandledExceptionError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[exception name],NSLocalizedDescriptionKey,[exception reason],NSLocalizedFailureReasonErrorKey,underlyingError,NSUnderlyingErrorKey,nil]]];
...
- (void)buildRequestHeaders
{
	...
	if (![[self requestHeaders] objectForKey:@"User-Agent"]) {
		...
		if (!userAgentString) {
			userAgentString = [JMCASIHTTPRequest defaultUserAgentString];
		}
		if (userAgentString) {
...
- (void)updatePartialDownloadSize
{
	...
	if ([self allowResumeForFileDownloads] && [self downloadDestinationPath] && [self temporaryFileDownloadPath] && [fileManager fileExistsAtPath:[self temporaryFileDownloadPath]]) {
		...
		if (err) {
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to get attributes for file at path '%@'",[self temporaryFileDownloadPath]],NSLocalizedDescriptionKey,error,NSUnderlyingErrorKey,nil]]];
			return;
		}
	}
}
...
- (void)startRequest
{
	...
	if ([self shouldStreamPostDataFromDisk] && [self postBodyFilePath] && [fileManager fileExistsAtPath:[self postBodyFilePath]]) {
		...
		if ([self compressedPostBodyFilePath] && [fileManager fileExistsAtPath:[self compressedPostBodyFilePath]]) {
			[self setPostBodyReadStream:[JMCASIInputStream inputStreamWithFileAtPath:[self compressedPostBodyFilePath] request:self]];
		} else {
			[self setPostBodyReadStream:[JMCASIInputStream inputStreamWithFileAtPath:[self postBodyFilePath] request:self]];
		}
		...
		if ([self postBody] && [[self postBody] length] > 0) {
			if ([self shouldCompressRequestBody] && [self compressedPostBody]) {
				[self setPostBodyReadStream:[JMCASIInputStream inputStreamWithData:[self compressedPostBody] request:self]];
			} else if ([self postBody]) {
				[self setPostBodyReadStream:[JMCASIInputStream inputStreamWithData:[self postBody] request:self]];
			}
			[self setReadStream:[(NSInputStream *)CFReadStreamCreateForStreamedHTTPRequest(kCFAllocatorDefault, request,(CFReadStreamRef)[self postBodyReadStream]) autorelease]];
	...
	if (![self readStream]) {
		[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to create read stream",NSLocalizedDescriptionKey,nil]]];
        return;
    }
	...
	[JMCASIHTTPRequest expirePersistentConnections];
	...
	if ([self shouldAttemptPersistentConnection]) {
		...
		CFReadStreamSetProperty((CFReadStreamRef)[self readStream], CFSTR("JMCASIStreamID"), [[self connectionInfo] objectForKey:@"id"]);
   ...
   // Start the HTTP connection
	...
	if (!streamSuccessfullyOpened) {
		...
		[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to start HTTP connection",NSLocalizedDescriptionKey,nil]]];
		return;	
	}
	...
	if (![self mainRequest]) {
		if ([self shouldResetUploadProgress]) {
			...
			[JMCASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:1];
		}
		if ([self shouldResetDownloadProgress] && ![self partialDownloadSize]) {
			[JMCASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:0 ofTotal:1];
		}
	}	
...
- (void)checkRequestStatus
{
	...
	if ([self shouldTimeOut]) {			
		...
		[self failWithError:JMCASIRequestTimedOutError];
		[self cancelLoad];
		[self setComplete:YES];
		[[self cancelledLock] unlock];
		return;
	...
	if ([self readStream]) {
		...
		if ([self postLength]) {
			...
			if (totalBytesSent > lastBytesSent) {
				...
				[self setLastActivityTime:[NSDate date]];
				[JMCASIHTTPRequest incrementBandwidthUsedInLastSecond:(unsigned long)(totalBytesSent-lastBytesSent)];		
...
// Used by JMCASINetworkQueue to create a HEAD request appropriate for this request with the same headers (though you can use it yourself)
- (JMCASIHTTPRequest *)HEADRequest
{
	JMCASIHTTPRequest *headRequest = [[self class] requestWithURL:[self url]];
...
- (void)setUploadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[JMCASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&uploadProgressDelegate withObject:nil amount:&max callerToRetain:nil];
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)setDownloadProgressDelegate:(id)newDelegate
{
	...
	double max = 1.0;
	[JMCASIHTTPRequest performSelector:@selector(setMaxValue:) onTarget:&downloadProgressDelegate withObject:nil amount:&max callerToRetain:nil];	
	#endif
	[[self cancelledLock] unlock];
}
...
- (void)updateDownloadProgress
{
	...
	[JMCASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[JMCASIHTTPRequest performSelector:@selector(request:didReceiveBytes:) onTarget:&downloadProgressDelegate withObject:self amount:&value callerToRetain:self];

	[JMCASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self totalBytesRead]+[self partialDownloadSize] ofTotal:[self contentLength]+[self partialDownloadSize]];
...
- (void)updateUploadProgress
{
	...
	[JMCASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&value callerToRetain:self];
	[JMCASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&value callerToRetain:self];
	[JMCASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self totalBytesSent]-[self uploadBufferSize] ofTotal:[self postLength]-[self uploadBufferSize]];
...
{
	[JMCASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[JMCASIHTTPRequest performSelector:@selector(request:incrementDownloadSizeBy:) onTarget:&downloadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
{
	[JMCASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&queue withObject:self amount:&length callerToRetain:self];
	[JMCASIHTTPRequest performSelector:@selector(request:incrementUploadSizeBy:) onTarget:&uploadProgressDelegate withObject:self amount:&length callerToRetain:self];
...
-(void)removeUploadProgressSoFar
{
	...
	[JMCASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&queue withObject:self amount:&progressToRemove callerToRetain:self];
	[JMCASIHTTPRequest performSelector:@selector(request:didSendBytes:) onTarget:&uploadProgressDelegate withObject:self amount:&progressToRemove callerToRetain:self];
	[JMCASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:0 ofTotal:[self postLength]];
...
+ (void)performSelector:(SEL)selector onTarget:(id *)target withObject:(id)object amount:(void *)amount callerToRetain:(id)callerToRetain
{
	if ([*target respondsToSelector:selector]) {
        ...
        NSMethodSignature *cbSignature = [JMCASIHTTPRequest methodSignatureForSelector:callback];
...
+ (void)updateProgressIndicator:(id *)indicator withProgress:(unsigned long long)progress ofTotal:(unsigned long long)total
{
	...
	[progressLock lock];
	[JMCASIHTTPRequest performSelector:selector onTarget:indicator withObject:nil amount:&progressAmount callerToRetain:nil];
	[progressLock unlock];
}
...
- (void)failWithError:(NSError *)theError
{
	...
	if (theError && [theError code] != JMCASIAuthenticationErrorType && [theError code] != JMCASITooMuchRedirectionErrorType) {
		[connectionsLock lock];
		#if DEBUG_PERSISTENT_CONNECTIONS
	...
	// If we have cached data, use it and ignore the error when using JMCASIFallbackToCacheIfLoadFailsCachePolicy
	if ([self downloadCache] && ([self cachePolicy] & JMCASIFallbackToCacheIfLoadFailsCachePolicy)) {
		if ([[self downloadCache] canUseCachedDataForRequest:self]) {
			[self useDataFromCache];
			return;
		}
	...
	JMCASIHTTPRequest *failedRequest = self;
	
	// If this is a HEAD request created by an JMCASINetworkQueue or compatible queue delegate, make the main request fail
	if ([self mainRequest]) {
...
- (void)readResponseHeaders
{
	...
	if ([self responseStatusCode] == 401) {
		[self setAuthenticationNeeded:JMCASIHTTPAuthenticationNeeded];
	} else if ([self responseStatusCode] == 407) {
		[self setAuthenticationNeeded:JMCASIProxyAuthenticationNeeded];
	} else {
		#if DEBUG_HTTP_AUTHENTICATION
	...
	if ([self useCookiePersistence]) {
		...
		for (cookie in newCookies) {
			[JMCASIHTTPRequest addSessionCookie:cookie];
		}
	}
	...
	if (![self willRedirect]) {
		...
		JMCASIHTTPRequest *theRequest = self;
		if ([self mainRequest]) {
...
- (BOOL)willRedirect
{
	...
	// Note that JMCASIHTTPRequest does not currently support 305 Use Proxy
...
- (void)saveProxyCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[JMCASIHTTPRequest saveCredentials:authenticationCredentials forProxy:[self proxyHost] port:[self proxyPort] realm:[self proxyAuthenticationRealm]];
	}	
}
...
- (void)saveCredentialsToKeychain:(NSDictionary *)newCredentials
{
	...
	if (authenticationCredentials) {
		[JMCASIHTTPRequest saveCredentials:authenticationCredentials forHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
	}	
}
...
- (NSMutableDictionary *)findProxyCredentials
{
	...
	JMCASIHTTPRequest *theRequest = [self mainRequest];
	// If this is a HEAD request generated by an JMCASINetworkQueue, we'll try to use the details from the main request
	...
	if ((!user || !pass)) {
		NSURLCredential *authenticationCredentials = [JMCASIHTTPRequest savedCredentialsForProxy:[self proxyHost] port:[self proxyPort] protocol:[[self url] scheme] realm:[self proxyAuthenticationRealm]];
		if (authenticationCredentials) {
...
- (NSMutableDictionary *)findCredentials
{
	...
	if ((!user || !pass) && useKeychainPersistence) {
		NSURLCredential *authenticationCredentials = [JMCASIHTTPRequest savedCredentialsForHost:[[self url] host] port:[[[self url] port] intValue] protocol:[[self url] scheme] realm:[self authenticationRealm]];
		if (authenticationCredentials) {
			user = [authenticationCredentials user];
			pass = [authenticationCredentials password];
			#if DEBUG_HTTP_AUTHENTICATION
...
- (void)failAuthentication
{
	[self failWithError:JMCASIAuthenticationError];
}
...
- (BOOL)showProxyAuthenticationDialog
{
	...
	if ([self shouldPresentProxyAuthenticationDialog]) {
		[JMCASIAuthenticationDialog performSelectorOnMainThread:@selector(presentAuthenticationDialogForRequest:) withObject:self waitUntilDone:[NSThread isMainThread]];
		return YES;
	}
	return NO;
	#else
...
- (void)attemptToApplyProxyCredentialsAndResume
{
	...
	if (!proxyAuthentication) {
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to get authentication object from response headers",NSLocalizedDescriptionKey,nil]]];
		return;
	}
	...
	if (proxyCredentials) {
		...
		// Something went wrong, we'll have to give up
			...
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply proxy credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
- (void)attemptToApplyCredentialsAndResume
{
	...
	if ([self authenticationNeeded] == JMCASIProxyAuthenticationNeeded) {
		[self attemptToApplyProxyCredentialsAndResume];
		return;
	}
	...
	if (requestCredentials) {
		...
		if ((([self authenticationScheme] != (NSString *)kCFHTTPAuthenticationSchemeNTLM) || [self authenticationRetryCount] < 2) && [self applyCredentials:requestCredentials]) {
			...
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileApplyingCredentialsType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Failed to apply credentials to request",NSLocalizedDescriptionKey,nil]]];
		}
...
{
	[self addRequestHeader:@"Authorization" value:[NSString stringWithFormat:@"Basic %@",[JMCASIHTTPRequest base64forData:[[NSString stringWithFormat:@"%@:%@",theUsername,thePassword] dataUsingEncoding:NSUTF8StringEncoding]]]];	
	...
	// If zero bytes were read, wait for the EOF to come.
		...
		if ([self isResponseCompressed] && ![self shouldWaitToInflateCompressedResponses]) {
			if (![self dataDecompressor]) {
				[self setDataDecompressor:[JMCASIDataDecompressor decompressor]];
			}
			NSError *err = nil;
		...
		[JMCASIHTTPRequest incrementBandwidthUsedInLastSecond:bytesRead];
...
{	
	...
	if ([self temporaryFileDownloadPath]) {
		...
		if ([self shouldRedirect] && [self needsRedirect] && [self allowResumeForFileDownloads]) {
			...
			if ([self shouldWaitToInflateCompressedResponses]) {
				[JMCASIDataDecompressor uncompressDataFromFile:[self temporaryFileDownloadPath] toFile:[self downloadDestinationPath] error:&fileError];
			...
			// Response should already have been inflated, move the temporary file to the destination path
				...
				if (moveError) {
					fileError = [NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to move file from '%@' to '%@'",[self temporaryFileDownloadPath],[self downloadDestinationPath]],NSLocalizedDescriptionKey,moveError,NSUnderlyingErrorKey,nil]];
				}
	...
	// If request has asked delegate or JMCASIAuthenticationDialog for credentials
...

{
	...
	if (![self error]) { // We may already have handled this error
		...
		[self cancelLoad];
		[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIConnectionFailureErrorType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:reason,NSLocalizedDescriptionKey,underlyingError,NSUnderlyingErrorKey,nil]]];
	} else {
		[self cancelLoad];
	}
	[self checkRequestStatus];
...
+ (BOOL)removeFileAtPath:(NSString *)path error:(NSError **)err
{
	...
	if ([fileManager fileExistsAtPath:path]) {
		...
		if (removeError) {
			if (err) {
				*err = [NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIFileManagementError userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"Failed to delete file at path '%@'",path],NSLocalizedDescriptionKey,removeError,NSUnderlyingErrorKey,nil]];
			}
			return NO;
		}
	}
...
- (BOOL)configureProxies
{
	...
	if (![self isPACFileRequest] && (![self proxyHost] && ![self proxyPort])) {
		...
		if (!proxies) {
			[self setReadStream:nil];
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:@"Unable to obtain information on proxy servers needed for request",NSLocalizedDescriptionKey,nil]]];
			return NO;
		}
...
- (void)fetchPACFile
{
	// For file:// urls, we'll use an async NSInputStream (JMCASIHTTPRequest does not support file:// urls)
	...
	// Create an JMCASIHTTPRequest to fetch the PAC file
	JMCASIHTTPRequest *PACRequest = [JMCASIHTTPRequest requestWithURL:[self PACurl]];
...
- (id)copyWithZone:(NSZone *)zone
{
	...
	JMCASIHTTPRequest *newRequest = [[[self class] alloc] initWithURL:[self url]];
...
{
	if (!sessionCookies) {
		[JMCASIHTTPRequest setSessionCookies:[[[NSMutableArray alloc] init] autorelease]];
	}
...
+ (void)addSessionCookie:(NSHTTPCookie *)newCookie
{
	...
	NSHTTPCookie *cookie;
	NSUInteger i;
	NSUInteger max = [[JMCASIHTTPRequest sessionCookies] count];
	for (i=0; i<max; i++) {
		cookie = [[JMCASIHTTPRequest sessionCookies] objectAtIndex:i];
		if ([[cookie domain] isEqualToString:[newCookie domain]] && [[cookie path] isEqualToString:[newCookie path]] && [[cookie name] isEqualToString:[newCookie name]]) {
			[[JMCASIHTTPRequest sessionCookies] removeObjectAtIndex:i];
			break;
		}
	}
	[[JMCASIHTTPRequest sessionCookies] addObject:newCookie];
	[sessionCookiesLock unlock];
}
...
+ (void)clearSession
{
	...
	[[[self class] defaultCache] clearCachedResponsesForStoragePolicy:JMCASICacheForSessionDurationCacheStoragePolicy];
}
...
+ (NSString *)defaultUserAgentString
{
	...
	// If we couldn't find one, we'll give up (and JMCASIHTTPRequest will use the standard CFNetwork user agent)
	if (!appName) {
		return nil;
	}
	...
	// We won't bother to check for systems prior to 10.4, since JMCASIHTTPRequest only works on 10.5+
    OSErr err;
...
- (void)performThrottling
{
	...
	[JMCASIHTTPRequest measureBandwidthUsage];
	if ([JMCASIHTTPRequest isBandwidthThrottled]) {
		[bandwidthThrottlingLock lock];
		// Handle throttling
...
+ (void)measureBandwidthUsage
{
	...
	if (!bandwidthMeasurementDate || [bandwidthMeasurementDate timeIntervalSinceNow] < -0) {
		[JMCASIHTTPRequest recordBandwidthUsage];
	}
...
+ (unsigned long)maxUploadReadLength
{
	...
	// We'll split our bandwidth allowance into 4 (which is the default for an JMCASINetworkQueue's max concurrent operations count) to give all running requests a fighting chance of reading data this cycle
...
{
	if (throttle) {
		[JMCASIHTTPRequest throttleBandwidthForWWANUsingLimit:JMCASIWWANBandwidthThrottleAmount];
	} else {
		[JMCASIHTTPRequest unsubscribeFromNetworkReachabilityNotifications];
		[JMCASIHTTPRequest setMaxBandwidthPerSecond:0];
		[bandwidthThrottlingLock lock];
...
{	
	...
	[JMCASIHTTPRequest registerForNetworkReachabilityNotifications];	
	[bandwidthThrottlingLock unlock];
	[JMCASIHTTPRequest reachabilityChanged:nil];
}
...
+ (void)registerForNetworkReachabilityNotifications
{
	[[JMCReachability reachabilityForInternetConnection] startNotifier];
	[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(reachabilityChanged:) name:kJMCReachabilityChangedNotification object:nil];
}


+ (void)unsubscribeFromNetworkReachabilityNotifications
{
	[[NSNotificationCenter defaultCenter] removeObserver:self name:kJMCReachabilityChangedNotification object:nil];
}

+ (BOOL)isNetworkReachableViaWWAN
{
	return ([[JMCReachability reachabilityForInternetConnection] currentReachabilityStatus] == ReachableViaWWAN);	
}

+ (void)reachabilityChanged:(NSNotification *)note
{
	[bandwidthThrottlingLock lock];
	isBandwidthThrottled = [JMCASIHTTPRequest isNetworkReachableViaWWAN];
	[bandwidthThrottlingLock unlock];
}
#endif
...
+ (void)setDefaultCache:(id <JMCASICacheDelegate>)cache
{
	[defaultCache release];
	defaultCache = [cache retain];
}

+ (id <JMCASICacheDelegate>)defaultCache
{
	return defaultCache;
}
...
+ (NSThread *)threadForRequest:(JMCASIHTTPRequest *)request
{
	if (!networkThread) {
...
#pragma mark blocks
#if NS_BLOCKS_AVAILABLE
- (void)setStartedBlock:(JMCASIBasicBlock)aStartedBlock
{
	[startedBlock release];
	startedBlock = [aStartedBlock copy];
}

- (void)setHeadersReceivedBlock:(JMCASIHeadersBlock)aReceivedBlock
{
	[headersReceivedBlock release];
	headersReceivedBlock = [aReceivedBlock copy];
}

- (void)setCompletionBlock:(JMCASIBasicBlock)aCompletionBlock
{
	[completionBlock release];
	completionBlock = [aCompletionBlock copy];
}

- (void)setFailedBlock:(JMCASIBasicBlock)aFailedBlock
{
	[failureBlock release];
	failureBlock = [aFailedBlock copy];
}

- (void)setBytesReceivedBlock:(JMCASIProgressBlock)aBytesReceivedBlock
{
	[bytesReceivedBlock release];
	bytesReceivedBlock = [aBytesReceivedBlock copy];
}

- (void)setBytesSentBlock:(JMCASIProgressBlock)aBytesSentBlock
{
	[bytesSentBlock release];
	bytesSentBlock = [aBytesSentBlock copy];
}

- (void)setDownloadSizeIncrementedBlock:(JMCASISizeBlock)aDownloadSizeIncrementedBlock{
	[downloadSizeIncrementedBlock release];
	downloadSizeIncrementedBlock = [aDownloadSizeIncrementedBlock copy];
}

- (void)setUploadSizeIncrementedBlock:(JMCASISizeBlock)anUploadSizeIncrementedBlock
{
	[uploadSizeIncrementedBlock release];
	uploadSizeIncrementedBlock = [anUploadSizeIncrementedBlock copy];
}

- (void)setDataReceivedBlock:(JMCASIDataBlock)aReceivedBlock
{
	[dataReceivedBlock release];
	dataReceivedBlock = [aReceivedBlock copy];
}

- (void)setAuthenticationNeededBlock:(JMCASIBasicBlock)anAuthenticationBlock
{
	[authenticationNeededBlock release];
	authenticationNeededBlock = [anAuthenticationBlock copy];
}
- (void)setProxyAuthenticationNeededBlock:(JMCASIBasicBlock)aProxyAuthenticationBlock
{
	[proxyAuthenticationNeededBlock release];
	proxyAuthenticationNeededBlock = [aProxyAuthenticationBlock copy];
}
- (void)setRequestRedirectedBlock:(JMCASIBasicBlock)aRedirectBlock
{
