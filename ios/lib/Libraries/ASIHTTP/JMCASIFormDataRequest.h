...
#import "JMCASIHTTPRequestConfig.h"

typedef enum _ASIPostFormat {
    JMCASIMultipartFormDataPostFormat = 0,
    JMCASIURLEncodedPostFormat = 1
	
} JMCASIPostFormat;

@interface JMCASIFormDataRequest : JMCASIHTTPRequest <NSCopying> {
	...
	JMCASIPostFormat postFormat;
	
	NSStringEncoding stringEncoding;
	
#if DEBUG_FORM_DATA_REQUEST
	// Will store a string version of the request body that will be printed to the console when JMCASIHTTPREQUEST_DEBUG is set in GCC_PREPROCESSOR_DEFINITIONS
...
@property (assign) JMCASIPostFormat postFormat;
