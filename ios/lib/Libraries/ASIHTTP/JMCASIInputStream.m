...
@implementation JMCASIInputStream

+ (void)initialize
{
	if (self == [JMCASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(JMCASIHTTPRequest *)theRequest
{
	JMCASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(JMCASIHTTPRequest *)theRequest
{
	JMCASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
// When throttling is on, we ask JMCASIHTTPRequest for the maximum amount of data we can read
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	[readLock lock];
	unsigned long toRead = len;
	if ([JMCASIHTTPRequest isBandwidthThrottled]) {
		toRead = [JMCASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	[JMCASIHTTPRequest incrementBandwidthUsedInLastSecond:toRead];
	[readLock unlock];
