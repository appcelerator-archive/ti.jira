...
// Private stuff
@interface JMCASIFormDataRequest ()
...
@implementation JMCASIFormDataRequest
...
- (id)initWithURL:(NSURL *)newURL
{
	...
	[self setPostFormat:JMCASIURLEncodedPostFormat];
...
- (void)addFile:(id)data withFileName:(NSString *)fileName andContentType:(NSString *)contentType forKey:(NSString *)key
{
	...
	if ([data isKindOfClass:[NSString class]]) {
		...
		if (!fileExists || isDirectory) {
			[self failWithError:[NSError errorWithDomain:JMCNetworkRequestErrorDomain code:JMCASIInternalErrorWhileBuildingRequestType userInfo:[NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"No file exists at %@",data],NSLocalizedDescriptionKey,nil]]];
		}
		...
		if (!contentType) {
			contentType = [JMCASIHTTPRequest mimeTypeForFileAtPath:data];
		}
	}
...
- (void)buildPostBody
{
	...
	if ([self postFormat] == JMCASIURLEncodedPostFormat) {
...
- (void)buildURLEncodedPostBody
{
	...
	if ([[self fileData] count] > 0) {
		[self setPostFormat:JMCASIMultipartFormDataPostFormat];
		[self buildMultipartFormDataPostBody];
		return;
	}
...
{
	JMCASIFormDataRequest *newRequest = [super copyWithZone:zone];
